...
GL_MAP = {
    ...
    0x93B6: ("ASTC_8x6", 8, 6), 0x93B7: ("ASTC_8x8", 8, 8), 0x93BA: ("ASTC_10x8", 10, 8),
    0x93BB: ("ASTC_10x10", 10, 10), 0x9278: ("ETC2", 0, 0), 0x8D64: ("ETC1", 0, 0),
    0x8058: ("RGBA8", 0, 0), 0x8051: ("RGB8", 0, 0)
}
...
class Engine:
    def decode_file(self, file_path, filename, progress_callback=None):
        ...
        if progress_callback: progress_callback("Чтение данных файла...", 5)
        ...
        if zstd_pos != -1:
            if progress_callback: progress_callback("Декомпрессия ZSTD контейнера...", 15)
            sig = "SCTX_ZSTD"
            header_data = raw_data[:zstd_pos]
            dctx = zstd.ZstdDecompressor()
            try:
                decompressed_data = dctx.decompress(raw_data[zstd_pos:], max_output_size=250_000_000)
            except Exception as e:
                print(f"ZSTD error: {e}")
        else:
            try:
                if progress_callback: progress_callback("Проверка сигнатур старых LZMA архивов...", 15)
        ...
        if not offsets and sig == "SCTX_ZSTD":
            if progress_callback: progress_callback("Сканирование бесструктурных текстур...", 40)
            ...
            for i in range(total_tex):
                ...
                if progress_callback: progress_callback(f"Распаковка графического блока {i+1}/{total_tex}...", percent)
        ...
        if progress_callback: progress_callback("Финальная упаковка архива ZIP...", 95)
    ...
    def encode_file(self, zip_path, progress_callback=None):
        ...
        if progress_callback: progress_callback("Извлечение ZIP архива...", 10)
        ...
        for i, tex in enumerate(layout["textures"]):
            percent = 20 + int((i / total_tex) * 70)
            if progress_callback: progress_callback(f"Рекомпрессия {tex['name']}...", percent)
        ...
        if progress_callback: progress_callback("Сборка выходного SCTX/SC файла...", 95)
